...
class KAN(nn.Module):
    def __init__(self, layers_hidden, grid_size=5):
        ...
        self.basis_coeff = nn.Parameter(torch.tensor(0.0))

    def forward(self, x):
        out = x
        for layer in self.layers:
            out = layer(out)
        basis = torch.sin(np.pi * x[:, 0:1]) * torch.sin(np.pi * x[:, 1:2])
        return out + self.basis_coeff * basis
...
def compute_loss(model, pde):
    ...
    # Gradients must be enabled here!
...
# --- STRATEGY 1: ADAM FOR MLP ---
def train_adam(model, pde, name, epochs=2000):
    print(f"\n--- Training {name} (Adam) ---")
    optimizer = optim.Adam(model.parameters(), lr=1e-3)
    scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.999) # Slow decay
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            # For Logging: Calculate Error (No Grad needed)
            with torch.no_grad():
                u_pred = model(pde.xy)
...
# --- STRATEGY 2: L-BFGS + GRID EXTENSION FOR KAN ---
def train_lbfgs_kan(model, pde, name, total_steps=100):
    ...
    for step in range(total_steps):
        ...
        if step in grid_schedule:
            ...
            # Re-init optimizer because params changed
            optimizer = optim.LBFGS(model.parameters(), lr=1.0, history_size=10, line_search_fn="strong_wolfe")
        ...
        if step % 5 == 0:
            # FIX: Compute Loss OUTSIDE no_grad (requires graph), Compute Error INSIDE no_grad
            ...
            history['step'].append(step) # Note: 1 LBFGS step ~ 20 Adam steps in compute
...
def plot_comparisons(hist_mlp, hist_kan):
    ...
    # Plot MLP (Adam)
    # Scale x-axis to match roughly in terms of "Compute Effort"
    # 2000 Adam epochs vs 100 LBFGS steps.
    plt.semilogy(hist_mlp['error'], label='MLP (Adam)', color='gray', linestyle='--')
    
    # Plot KAN on secondary axis or just overlay if we normalize time?
    # Let's just plot them against their own iterations but label clearly
    # We will interpolate KAN to stretch it across the graph for visual comparison
    kan_x = np.linspace(0, len(hist_mlp['error']), len(hist_kan['error']))
    plt.semilogy(kan_x, hist_kan['error'], label='KAN (L-BFGS)', color='blue', linewidth=2)
    
    plt.title("PDE Solver: MLP (Adam) vs KAN (L-BFGS)")
    plt.xlabel("Relative Training Progress")
    plt.ylabel("L2 Relative Error")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig("pde_adam_vs_lbfgs.png")
...
if __name__ == "__main__":
    ...
    hist_kan = train_lbfgs_kan(kan, pde, "KAN", total_steps=100)
